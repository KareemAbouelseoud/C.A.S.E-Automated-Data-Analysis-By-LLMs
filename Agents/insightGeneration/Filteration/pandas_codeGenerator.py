...
def run_pandas_Coder_agent(state:dict) -> list[InsightCard]:
    ...
    for card in state["insight_cards"]:
        ...
        card.resulted_df = str(global_dict['resulted_df'].to_json()) if global_dict['resulted_df'] is not None or global_dict["resulted_df"].empty==False else ""
    return state["insight_cards"]

def run_pandas_Coder_agent_advanced_card(filtered_df:pd.DataFrame,card:InsightCard) -> InsightCard:
    """Generates a Pandas DataFrame agent based on the provided Insight Card."""
