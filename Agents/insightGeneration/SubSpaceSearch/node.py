...
def subspace_search(df:pd.DataFrame,card:InsightCard,desc:DataDescription, beam_width=3, max_depth=2, exp_factor=2):
    ...
    beam = [(So, card, 0)]  # Initialize beam with the original card and a penalty counter
    best_score = card.Score
    penalty_counter = 0
    all_insights = {} # Dictionary to store all insights with the same score

    for depth in range(1, max_depth + 1):
        # print("Original QUGEN CARD",card.question)
        new_beam = []
        for S, _card, penalty in beam:
            ...
            for i in range(exp_factor):
                ...
                for df_index,_filtered_df in enumerate(filtered_dfs):
                    ...
                    try:
                        ...
                        if Advanced_Card.Score > best_score:
                            best_score = Advanced_Card.Score
                            penalty = 0  # Reset penalty if score increases
                        else:
                            penalty = penalty + 1  # Increment penalty if score doesn't increase
                        
                        if penalty < 3:  # Only keep subspaces with a penalty less than 3
                            new_beam.append((_new_subspace, Advanced_Card, penalty))  # Save the new beam with the best attributes
                            print ("adding to the beam")
                            beam.append((_new_subspace, Advanced_Card, penalty))  # Save the new beam with the best attributes
                        else:
                            print(f"Discarding subspace due to penalty: {Snew}")
                    except:
                        # print(f"Warning: Skipping current Advanced Card as - no suggested cards available.")
                        continue
                # for _ss in beam:
        ...
        # Collect insights with the same score
        for S, card, penalty in new_beam:
            score = card.Score
            if score not in all_insights:
                all_insights[score] = []
            all_insights[score].append((S, card))
        
        # Truncate beam
        new_beam.sort(key=lambda x: x[1].Score, reverse=True)
        beam = new_beam[:beam_width]
        
    # Sort insights by score in descending order
    sorted_scores = sorted(all_insights.keys(), reverse=True)
